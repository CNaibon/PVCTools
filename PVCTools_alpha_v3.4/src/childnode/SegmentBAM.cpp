...
int ParallelBAM(int argc, char *argv[])
{
    ...
    for (int j = 0; j < FileNumber; j++)
    {
        ...
        ReadCount[j + 1] = ReadCount[j] + LNCount[j]  - (long)((int)(ceil(Reserved / Maxlen_PreLine))*Maxlen_PreLine);
        if(j == FileNumber - 1)
        {
            ...
            getline(fp_sp, TransBuffer);
            fp_sp.close();
            LNCount[j] = LNCount[j] - Maxlen_PreLine + atol(TransBuffer.c_str()) - 1;
...
#pragma omp parallel for
    for (int i = 0; i < (int)ChrName.size(); ++i)
    {
        for (int n = 0; n < (int)SampleName.size(); ++n)
        {
            ...
            fputs(ModCommand, fp_trans);
            snprintf(ModCommand, sizeof(ModCommand), "\n%s ParallelBAM -w %s -c %s -b %s -f %d -T %s\n", argv[0], PathWork, ChrName[i].c_str(), SampleName[n].c_str(), FileNumber[i], Tool.c_str());
            fputs(ModCommand, fp_trans);
            fclose(fp_trans);
    ...
    ifstream fp_tmp;
    string Buffer = "0";
    while (atol(Buffer.c_str()) != TotalNumber)
    {
        sleep(1000);
        snprintf(Command, sizeof(Command), "wc -l %s/SortCount > %s/SortCount_tmp", PathWork, PathWork);
        system(Command);
        snprintf(Command, sizeof(Command), "%s/SortCount_tmp", PathWork);
