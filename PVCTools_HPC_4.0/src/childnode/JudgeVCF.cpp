...
#pragma omp parallel for
    for (int i = 0; i < (int)ChrName.size(); ++i)
    {
        ...
        snprintf(Command, sizeof(Command), "touch %s/%s_tmp", PathWork, ChrName[i].c_str());
        system(Command);
        //Count the current number of FA files.
        snprintf(Command, sizeof(Command), "ls -l %s/fa/%s |grep \"^-\"| grep '.fa$' |wc -l > %s/%s_tmp", PathWork, ChrName[i].c_str(), PathWork, ChrName[i].c_str());
        system(Command);
        snprintf(Command, sizeof(Command), "%s/%s_tmp", PathWork, ChrName[i].c_str());
        ...
        snprintf(Command, sizeof(Command), "wc -L %s/fa/%s.fa > %s/%s_tmp", PathWork, PathWork, ChrName[i].c_str(),
                 ChrName[i].c_str());
        system(Command);
        snprintf(Command, sizeof(Command), "%s/%s_tmp", PathWork, ChrName[i].c_str());
        fp_chr.open(Command,ios::in);
        ...
        for (int j = 0; j < FileNumber[i]; j++)
        {
            ...
            snprintf(Command, sizeof(Command), "wc -l %s/fa/%s/%s_%d.fa > %s/%s_tmp", PathWork, ChrName[i].c_str(), ChrName[i].c_str(), j, PathWork, ChrName[i].c_str());
            system(Command);
            snprintf(Command, sizeof(Command), "%s/%s_tmp", PathWork, ChrName[i].c_str());
            fp_chr.open(Command,ios::in);
            getline(fp_chr, VCFBuffer);
            ...
            if(j == FileNumber[i] -1)
            {
                snprintf(Command, sizeof(Command), "tail -n 1 %s/fa/%s/%s_%d.fa | wc -c > %s/%s_tmp", PathWork, ChrName[i].c_str(), ChrName[i].c_str(), j, PathWork, ChrName[i].c_str());
                system(Command);
                snprintf(Command, sizeof(Command), "%s/%s_tmp", PathWork, ChrName[i].c_str());
                fp_chr.open(Command,ios::in);
                getline(fp_chr, VCFBuffer);
    ...
    snprintf(ShellCommand, sizeof(ShellCommand), "%s/FinishTime_%ld", PathWork, FinishTime);
    timecount.open(ShellCommand, ios::out|ios::app);
    timecount.close();
