...
int Sam_Address_Modify(char *file_name, long address_count, const char *chr_name, long LN)
{
    ...
    while (!fp_old.eof())
    {
        if(Modify(Buffer, address_count, chr_name, LN) != -1) fp_new<<Buffer<<endl;
        getline(fp_old,Buffer);
    }
    fp_old.close();
    fp_new.close();
...
//#pragma omp parallel for
    ...
    snprintf(ShellCommand, sizeof(ShellCommand), "touch %s/SortCount", PathWork);
    system(ShellCommand);

    cerr<<"begin sort sh parallel"<<endl;
    //Parallel Computing
#pragma omp parallel for
    for (int i = 0; i < (int)ChrName.size(); ++i)
    {
...
//            }
            ...
            system(ModCommand);
            snprintf(ModCommand, sizeof(ModCommand), "mkdir -p %s/sub_script/segmentBAM/%s/%s_%s", PathWork, SampleName[n].c_str(), SampleName[n].c_str(), ChrName[i].c_str());
            system(ModCommand);

            //printf("BAM file to extract...\n");
            snprintf(ModCommand, sizeof(ModCommand), "%s/sub_script/segmentBAM/%s/%s_%s/%s_%s.pbs", PathWork, SampleName[n].c_str(), SampleName[n].c_str(), ChrName[i].c_str(), SampleName[n].c_str(), ChrName[i].c_str());
