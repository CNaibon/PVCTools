...
int Submit(int argc, char *argv[])
{
    ...
    for (int i = 0; i < (int)ChrName.size(); ++i)
    {
        ...
        snprintf(Command, sizeof(Command), "rm -rf %s/%s_tmp ", PathWork, ChrName[i].c_str());
        system(Command);
    }
    ...
    system(ShellCommand);
    snprintf(ShellCommand, sizeof(ShellCommand), "mkdir -p %s/sub_script/submit", PathWork);
    system(ShellCommand);
...
#pragma omp parallel for
    for (int i = 0; i < (int)ChrName.size(); i++)
    {
        ...
        {
            FILE *fp_sh;
            snprintf(Command, sizeof(Command), "%s/sub_script/submit/%s_%d_%s.pbs", PathWork, ChrName[i].c_str(), k, Tool.c_str());
...
//            fputs(Command, fp_sh);
            ...
            //Submit.
//            snprintf(Command, sizeof(Command), "qsub < %s/sub_script/submit/%s_%d_%s.pbs", PathWork, ChrName[i].c_str(), k, Tool.c_str());
//            system(Command);
        }
    }
